...
The main branch of this repo will point to the latest tested REDCap release by default.  If insated you would like to test REDCap changes that have not yet made it into a release, checkout the `dev` branch for `redcap_cypress_docker`.  If you would like to test an older REDCap version, perform the followings step:
